...
public class Logger {
  ...
  private static final String PRODUCT_ID = "WindowTester Pro";

  /**
   * A print writer wrapping System.out. Override the close method so that it cannot be closed.
   */
  private static final PrintWriter sysOut =
   ...
   *
   * @param aMessage the message to be logged
   * @deprecated use your own logger instead
   */
  ...
  /**
   * Print the message for a log entry to the specified file
   *
   * @param writer   the log writer (not <code>null</code>)
   * @param aMessage the message to be logged
   */
