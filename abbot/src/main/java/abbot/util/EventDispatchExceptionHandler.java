...
public class EventDispatchExceptionHandler implements Thread.UncaughtExceptionHandler {
  ...
  public void install() {
    if (SwingUtilities.isEventDispatchThread()) {
      throw new IllegalStateException(
          "Handler must not be installed from the event dispatch thread");
    }
    ...
    try {
      ...
      String msg =
          "Exception handler (" + cls + ") must have an accessible no-args Constructor: " + e;
  ...
  private void handleException(String threadName, Throwable thrown) {
    ...
    if (thrown instanceof DummyException) {
      ...
      if (!handled) {
        Log.debug("Handling exception on event dispatch thread: " + thrown + " with " + getClass());
        Log.debug(thrown);
