 ...
 * equivalent to the system property java.class.path.<p> The class loader may optionally load a class <i>before</i> the
...
public class AppClassLoader extends NonDelegatingClassLoader {
  ...
  /**
   * This class loader is used to load bootstrap classes that must be preloaded.
   */
  ...
  public AppClassLoader(String classPath) {
    ...
    extensionsLoader =
        new NonDelegatingClassLoader(
            System.getProperty("java.class.path"), AppClassLoader.class.getClassLoader()) {
  ...
  public boolean isEventDispatchThread() {
    return (eventQueue != null && Thread.currentThread() == eventQueue.thread)
  ...
  /**
   * Finds and loads the class with the specified name from the search path.  If the class is a bootstrap class and
   * must be preloaded, use our own bootstrap loader.  If it is an extension class, use our own extensions loader.
   *
  ...
  /**
   * Ensure that everything else subsequently loaded on the same thread or any subsequently spawned threads uses the
   * given class loader.  Also ensure that classes loaded by the event dispatch thread and threads it spawns use the
   * given class loader.
   */
  public void install() {
    ...
    if (installedThread != null) {
      String msg = Strings.get("appcl.already_installed", new Object[] {installedThread});
