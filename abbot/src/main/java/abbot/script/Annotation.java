...
public class Annotation extends Step {
  ...
  private static final String USAGE =
      "<annotation [title=\"...\"] [component=\"<component ID>\"] [x=XX y=YY] [width=WWW"
          + " height=HHH] [userDismiss=\"true\"]>Text or HTML message</annotation>";

  private static final int WORD_SIZE = 6;
  private static final Color BACKGROUND =
      new Color(
          (Color.yellow.getRed() + Color.white.getRed() * 3) / 4,
  ...
  private AnnotationWindow createWindow() {
    ...
    if (w == null) {
      f = new Frame();
      w = (title != null) ? new AnnotationWindow(f, title) : new AnnotationWindow(f);
    }
