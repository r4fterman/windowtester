...
/**
 * Provides encapsulation of a location on a JTabbedPane (notably a tab). Use the JTabbedPaneLocation#JTabbedPaneLocation(Point)
 * ctor to indicate a specific coordinate.
 */
  ...
  public Point getPoint(Component c) {
    ...
    if (tabName != null) {
      if ((idx = tabNameToIndex(tabs, tabName)) == -1) {
        String msg = Strings.get("tester.JTabbedPane.invalid_name", new Object[] {tabName});
        throw new LocationUnavailableException(msg);
      }
    }
    if (idx != -1) {
