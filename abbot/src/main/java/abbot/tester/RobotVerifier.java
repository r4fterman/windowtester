...
public class RobotVerifier {
  ...
  /**
   * Auto-detect whether the robot actually works. Use this to tell whether we're in w32 pseudo-headless mode, such as
   * when in a remote shell (ssh) or running as a service.  This is also used as a delay mechanism to ensure the AWT
   * subsystem is running before continuing (particularly a problem on early OSX 1.4 VM versions).
   *
  ...
  public static boolean verify(java.awt.Robot robot) {
    ...
    if (Bugs.hasMissingWindowMouseMotion()) {
      ...
      try {
        ...
        w.getClass().getDeclaredMethod("setUndecorated", boolean.class).invoke(w, Boolean.TRUE);
      } catch (Exception e) {
      }
    } else {
      w = new Window(f);
    ...
    try {
      do {
        w.toFront();
        synchronized (flag) {
          robot.mouseMove(w.getX(), w.getY() + w.getHeight() - 1);
          robot.mouseMove(w.getX() + 1, w.getY() + w.getHeight() - 2);
          try {
            flag.wait(500);
