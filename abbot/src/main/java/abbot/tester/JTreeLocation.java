...
/**
 * Provides encapsulation of a visible location on a {@link JTree}. A row index or a {@link String}ified {@link
 * TreePath} (i.e. each {@link TreePath} component is a {@link String}) may be used to indicate the location.  Note that
 * if a {@link TreePath} is used, it must be visible at the time the location is used in order to be valid.
 */
...
public class JTreeLocation extends ComponentLocation {
  ...
  /**
   * Create an uninitialized JTreeLocation.  If queried for its {@link Point} will return the default location.
   */
    ...
    if (o instanceof JTreeLocation loc) {
      ...
      if (treePath != null) {
        ...
        for (int i = 0; i < treePath.getPathCount(); i++) {
          ...
          } else if (!treePath.getPathComponent(i).equals(loc.treePath.getPathComponent(i))) {
            return false;
          }
        }
        return true;
  ...
  /**
   * Return whether the given Object matches the final element of the given TreePath.
   */
  ...
  /**
   * Given a {@link TreePath} of {@link String}s, return the equivalent {@link TreePath} for the given {@link JTree}
   * constructed from objects from the tree's model.
   *
  ...
  /**
   * Build up the given tree path with Objects from the TreeModel which match the given array of Objects.
   */
  private static TreePath findMatchingPath(JTree tree, TreePath realPath, Object[] input) {
    ...
    Log.debug(
        "Comparing " + realPath + " with " + input[0] + " from " + java.util.Arrays.asList(input));
    ...
    if (!matchesLastComponent(tree, input[0], realPath)) {
      ...
      for (int i = 0; i < count; i++) {
        ...
        TreePath newPath = findMatchingPath(tree, realPath.pathByAddingChild(child), subs);
        if (newPath != null) {
          return newPath;
        }
      }
