...
/**
 * Provides recording of raw AWT events and high-level semantic events. This is the main controller for any
 * SemanticRecorder objects.
 */
...
public class EventRecorder extends Recorder implements SemanticEvents {
      ...
      if (isKeyString(step)) {
        ...
        while (++j < steps.size()) {
          ...
          if (isKeyString(next)) {
            ...
            setStatus("Joining '" + args1[1] + "' and '" + args2[1] + "'");
            steps.remove(j--);
          } else {
  ...
  /**
   * Handle an event.  This can either be ignored or contribute to the recording. For a given event, if no current
