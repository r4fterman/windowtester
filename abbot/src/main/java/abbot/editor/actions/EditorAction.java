...
public abstract class EditorAction extends AbstractAction implements EditorConstants {
  ...
  /**
   * Key to refer to the action in ActionMaps; <strong>not</strong> the same as the NAME property which is typically
   * used for labels.
   */
  ...
  public EditorAction(String actionKey) {
    ...
    if (!"".equals(accelerator) && accelerator != null) {
      accelerator = abbot.tester.AWTConstants.MENU_SHORTCUT_STRING + accelerator;
      try {
    ...
    if (!"".equals(mnemonic) && mnemonic != null) {
      ...
      try {
        ...
        String msg =
            Strings.get(
                "editor.bad_mnemonic",
                new Object[] {mnemonic, key + ".mn", java.util.Locale.getDefault(), e.toString()});
   ...
   * @param size Size in pixels of the icon from the filename, or zero if none, e.g. 16 for help16.gif or 0 for
   *             help.gif.
